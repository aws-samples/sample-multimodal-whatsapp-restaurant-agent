#!/bin/bash
# Preflight for the WhatsApp Restaurant AI Host.
#
# Validates developer-machine prerequisites:
...
#   - Bedrock Nova model access granted (warn-only if missing)
#   - Bedrock AgentCore Runtime supported AZs reachable (warn-only)
#   - assertion: python/pip/poetry/uv/etc. are NOT required at the dev layer.
#
# This variant has NO Chime / SIP / PSTN number, so there is NO phone-number
# quota check. The Meta Business account, WhatsApp Business Account, phone
# number, and Calling API access are a MANUAL prerequisite performed in the
# Meta and AWS consoles; they are required only for live (non-mocked) testing
# and are intentionally NOT probed here (no Meta credentials live on the dev
# machine, and the deploy itself does not need them).
#
...
fail() { echo -e "${RED}[FAIL] $1${NC}"; [ -n "${2-}" ] && echo -e "   ${YELLOW}-> $2${NC}"; FAIL=$((FAIL+1)); }
...
# CDK Bootstrap. The SSM parameter is the real source of truth CDK deploy
# checks (not the CloudFormation stack). Minimum required version is 6.
if [ "$CDK_AVAILABLE" = true ] && [ "$AWS_CREDS_OK" = true ]; then
  ...
  if [ -n "$BOOTSTRAP_VERSION" ] && [ "$BOOTSTRAP_VERSION" -ge "$MIN_BOOTSTRAP_VERSION" ] 2>/dev/null; then
    ...
    if [ -n "$STACK_EXISTS" ] && [ -z "$BOOTSTRAP_VERSION" ]; then
      ...
      echo -e "   that does not include the version tracking CDK needs to deploy."
      echo ""
...
# Bedrock Nova model access (warn-only - the runtimes need it at runtime, not
# at deploy time, so we do not block). Probes for Nova Sonic (voice + voice
# notes) and Nova Pro (multimodal chat).
if [ "$AWS_CLI_OK" = true ] && [ "$AWS_CREDS_OK" = true ]; then
  ...
  if echo "$NOVA_MODELS" | grep -q "nova-sonic"; then
    pass "Bedrock Nova Sonic access granted in $REGION (voice + voice notes)"
  else
    warn "Bedrock Nova Sonic access not granted in $REGION - request it in the Bedrock console before voice testing"
  fi
  if echo "$NOVA_MODELS" | grep -q "nova-pro"; then
    pass "Bedrock Nova Pro access granted in $REGION (multimodal chat)"
  else
    warn "Bedrock Nova Pro access not granted in $REGION - request it in the Bedrock console before chat testing"
  fi
fi
...
# randomized per account. We probe and warn if fewer than 2 supported AZs are
# reachable. deploy-all.sh also enforces this as a hard error (exit 6).
if [ "$AWS_CLI_OK" = true ] && [ "$AWS_CREDS_OK" = true ]; then
  REGION=$(aws configure get region 2>/dev/null || echo "us-east-1")
...
info "Each CDK dir has its own node_modules - deploy-all.sh --low-storage-mode cleans other dirs between deploys to stay under CloudShell's 1 GB home-dir limit. If you are NOT on CloudShell, ignore."
