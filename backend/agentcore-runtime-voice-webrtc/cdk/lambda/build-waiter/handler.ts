 ...
 * and build-complete-waiter pattern (carried over from the VoiceNotes / Chat
 * Runtime build pipelines; identical logic).
 *
 ...
 *      does a `codebuild:BatchGetBuilds` on the buildId; returns
 *      `IsComplete: true` only on terminal `SUCCEEDED`. Any FAILED / STOPPED /
 *      FAULT / TIMED_OUT state raises so CFN rolls the custom resource back
 *      (which fails the stack) with the CodeBuild log-stream URL in the error.
 *   3. On Delete, the handler is a no-op (buildId is a historical artifact;
 *      the ECR lifecycle policy on the image repo handles cleanup).
 *
 * No phone number (E.164) is ever touched here. All logging goes to CloudWatch
 * via console.*; the Provider framework captures stdout.
 */
...
export async function isComplete(event: IsCompleteRequest): Promise<IsCompleteResponse> {
  ...
  switch (status) {
    ...
    default:
      ...
      // including the CloudWatch Logs link.
      throw new Error(
