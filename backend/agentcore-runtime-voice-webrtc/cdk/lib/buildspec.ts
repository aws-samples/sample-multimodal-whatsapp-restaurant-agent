/**
 * CodeBuild buildspec for the WhatsApp Call Runtime image.
 *
 * Mirrors the VoiceNotes / Chat Runtime buildspec:
 ...
 *
 * The Call Runtime image carries the WebRTC media stack (aiortc + av) plus its
 * native deps (see agent/Dockerfile). The build runs entirely inside
 * CodeBuild's ARM64 standard image; no Python on the developer workstation is
 * ever invoked.
 *
 * Env vars the project injects (set in voice-webrtc-stack.ts):
