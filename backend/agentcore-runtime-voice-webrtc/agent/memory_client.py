...
This is the ONE memory client imported by all three runtimes (Call, VoiceNotes,
Chat). It talks to the single shared Amazon Bedrock AgentCore Memory resource
provisioned by backend/agentcore-memory (MemoryStack), whose id/ARN are passed
into each runtime as environment variables sourced from CloudFormation
parameters (R18.1).
