#!/usr/bin/env node
/**
 * Synthetic Data Population for the WhatsApp Restaurant AI Host.
 *
 * Ported + adapted from the telephony repo's backend/synthetic-data/populate-data.js.
 * Changes from the telephony version:
 *   - Customer identity comes from --user-name + --user-phone (E.164). Email is
 *     synthesized.
 *   - customerId is derived the same way the live runtimes derive it:
 *     `wa-<sha256(e164 || pepper)[:16]>` (19 chars) with the pepper read from
 *     SSM at `/${prefix}/customer-id-pepper` (default `/qsr-wa/...`, the same
 *     source the worker/runtimes read). Guarantees the Customers PK we write
 *     matches the wa- id the runtimes compute for that phone number.
 *   - Table names read from cdk-outputs/wa-ddb.json, keyed on the logical
 *     stack id `DynamoDBStack`.
 ...
 *     --deployment-prefix qsr-wa \
 *     --non-interactive
 *
 * To seed Menu + Locations only (anonymous demo, no PII), omit --user-phone.
 */
const fs = require('fs');
...
Optional:
  ...
  --business-name STRING    Business search term (e.g. "burgers").
                            Required in --non-interactive mode.
  --deployment-prefix PFX   Pepper SSM path prefix (default: qsr-wa).
                            Resolves to /\${prefix}/customer-id-pepper.
  --non-interactive         Skip all interactive prompts.

Reads table names from cdk-outputs/wa-ddb.json (keyed on "DynamoDBStack").
`);
        process.exit(0);
    }
  }
...
function loadTableNames(workspaceRoot) {
  const outputsPath = path.join(workspaceRoot, 'cdk-outputs', 'wa-ddb.json');
  if (!fs.existsSync(outputsPath)) {
    fail(`Deployment outputs not found at: ${outputsPath}`);
    info('Run the wa-ddb layer of scripts/deploy-all.sh first.');
    return null;
  }
  let outputs;
  try {
...
async function main() {
  ...
  header('WhatsApp Restaurant AI Host - Synthetic Data');
  ...
  ok(`Tables resolved from cdk-outputs/wa-ddb.json`);
  ...
  if (seedLoyalty) {
    ...
    info('Message (or call) the WhatsApp business number from this phone to test the loyalty greeting + memory recall path.');
  } else {
    ok(`Seeded ${locations.length} locations, ${menuItems.length} menu items (no loyalty customer)`);
    info('Anonymous customers can order end-to-end. To add a loyalty customer later, re-run:');
    info('  node backend/synthetic-data/populate-data.js --user-name "Jane Doe" --user-phone +1... --location "..." --business-name "..." --non-interactive');
  }
