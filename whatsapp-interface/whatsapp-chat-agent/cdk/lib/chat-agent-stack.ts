...
/**
 * {prefix}-ChatAgentStack - the WhatsApp Chat Runtime (Task 7.1).
 *
 * One self-contained CDK app that provisions, in a single stack, the complete
 * build-and-host pipeline for the multimodal Chat Runtime:
 *
 *   1. ECR repository      `{prefix}-wa-chat-agent` - holds the ARM64 image.
 *   2. S3 source bucket    + BucketDeployment of the sibling `agent/` dir.
 *   3. CodeBuild project   `{prefix}-wa-chat-agent-build` - ARM64 Docker build
 ...
 *   5. AgentCore Runtime   `AWS::BedrockAgentCore::Runtime` (the WhatsApp Chat
 *      Runtime), default/managed (PUBLIC) network mode.
 *
 * Why one stack (vs the telephony repo's three: ecr / build / runtime): the
 * design's project structure pins a single `lib/chat-agent-stack.ts` for this
 * module, and consolidating removes the cross-stack ECR-URI / build-waiter-ARN
 * CfnParameter threading the telephony split needed. The internal wiring is
 * otherwise identical to the telephony pattern.
 *
 * Network mode (design "Region and AZ constraints" + "WhatsApp Chat Runtime"):
 * the Chat Runtime is a request/response TEXT runtime (Strands + Converse +
 * Nova Pro). It uses the DEFAULT/managed AgentCore network mode (PUBLIC) - the
 * same mode the telephony runtime settled on once it no longer needed outbound
 * UDP. It is explicitly NOT the VPC-only mode: that mode exists for the Call
 * Runtime's outbound-UDP-to-TURN media need (design: the VPC network mode is
 * "required because PUBLIC mode does not support outbound UDP"). The Chat
 * Runtime has no UDP/TURN/WebRTC path, so managed mode is correct and avoids
 * provisioning a VPC/NAT it would never use. Its only outbound traffic is HTTPS
 * to AWS service APIs (Bedrock Converse, CloudWatch, the AgentCore Gateway, the
 ...
 *  - OUTPUT CfnOutputs (NO exportName): ChatRuntimeArn (consumed by the webhook
 *    stack as ChatRuntimeArn), ChatAgentEcrRepoUri, plus build/AZ observability.
 *
...
export class ChatAgentStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const agentCoreGatewayUrl = new cdk.CfnParameter(this, 'AgentCoreGatewayUrl', {
      ...
      minLength: 1,
      description:
        'AgentCore Gateway MCP URL (from the gateway stack output). The Chat Runtime reaches every backend tool through this gateway only (R4.7); passed into the container as an environment identifier.',
    });
    ...
    // constraints"). The VPC-mode runtimes (Call, VoiceNotes) bind a private
    // subnet with a NAT gateway in one of these AZs. The design states the
    // Chat Runtime "inherits the same AZ-resolution contract".
    //
    // For THIS runtime the contract is accepted but NOT wired into a network
    // configuration: the Chat Runtime uses the default/managed (PUBLIC) network
    // mode, where AgentCore resolves placement itself across its supported AZs.
    // We take the contract as a CfnParameter for cross-runtime uniformity (one
    // deploy-all.sh threading shape for all three runtimes) and echo it as an
    // output for operator observability. If this runtime is ever switched to
    // VPC mode, this is the single input that already carries the AZ contract.
    //
    // Shape matches the telephony network app's resolution: a comma-delimited
    // list of Bedrock-supported AZ IDs. Default is the documented us-east-1 set
    // so the stack synthesizes/deploys even when deploy-all.sh does not thread
    // the value.
    const agentCoreAzIds = new cdk.CfnParameter(this, 'AgentCoreAzIds', {
      ...
      description:
        'Shared AZ-resolution contract: comma-delimited Bedrock AgentCore-supported AZ IDs in us-east-1. The Chat Runtime runs in managed (PUBLIC) network mode and does not bind a subnet, so this is accepted for cross-runtime uniformity and echoed as an output (not wired into a network configuration).',
    });

    // ----------------- Prefixed physical names (Fn::Sub at deploy time) -----
    const ecrRepoName = cdk.Fn.sub('${P}-wa-chat-agent', { P: prefix });
    const buildProjectName = cdk.Fn.sub('${P}-wa-chat-agent-build', { P: prefix });
    const buildRoleName = cdk.Fn.sub('${P}-wa-chat-build-role', { P: prefix });
    const runtimeRoleName = cdk.Fn.sub('${P}-wa-chat-runtime-role', { P: prefix });
    ...
    // exists" (matches the telephony ecr-stack posture; flip to RETAIN +
    // emptyOnDelete:false for production image-history retention).
    const repo = new ecr.Repository(this, 'ChatAgentRepo', {
    ...
    const sourceBucket = new s3.Bucket(this, 'SourceBucket', {
      bucketName: cdk.Fn.sub('${P}-wa-chat-src-${A}-${R}', {
        P: prefix,
    ...
    // ../../agent (cdk/lib -> cdk -> whatsapp-chat-agent -> agent).
    const deployment = new s3deploy.BucketDeployment(this, 'ChatAgentSourceDeployment', {
    ...
    const buildRole = new iam.Role(this, 'BuildRole', {
      ...
      description:
        'Role assumed by the CodeBuild project that builds the WhatsApp Chat Runtime ARM64 image.',
    });
    ...
    const project = new codebuild.Project(this, 'ChatAgentBuild', {
      ...
      logging: {
        cloudWatch: {
          ...
          logGroup: new logs.LogGroup(this, 'BuildLogGroup', {
            logGroupName: cdk.Fn.sub('/aws/codebuild/${P}-wa-chat-agent-build', { P: prefix }),
    ...
    const waiterLogGroup = new logs.LogGroup(this, 'BuildWaiterLogGroup', {
      logGroupName: cdk.Fn.sub('/aws/lambda/${P}-wa-chat-build-waiter', { P: prefix }),
      retention: logs.RetentionDays.ONE_MONTH,
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });
    const waiterIsCompleteLogGroup = new logs.LogGroup(this, 'BuildWaiterIsCompleteLogGroup', {
      logGroupName: cdk.Fn.sub('/aws/lambda/${P}-wa-chat-build-waiter-check', { P: prefix }),
      retention: logs.RetentionDays.ONE_MONTH,
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });
    ...
    // next deploy (telephony build-stack rationale).
    ...
    const waiterFn = new NodejsFunction(this, 'BuildWaiterFn', {
      functionName: cdk.Fn.sub('${P}-wa-chat-build-waiter', { P: prefix }),
    ...
    const waiterIsCompleteFn = new NodejsFunction(this, 'BuildWaiterIsCompleteFn', {
      functionName: cdk.Fn.sub('${P}-wa-chat-build-waiter-check', { P: prefix }),
      entry: waiterHandlerEntry,
      handler: 'isComplete',
      runtime: lambda.Runtime.NODEJS_24_X,
    ...
    const runtimeRole = new iam.Role(this, 'ChatRuntimeRole', {
      roleName: runtimeRoleName,
      assumedBy: new iam.ServicePrincipal('bedrock-agentcore.amazonaws.com'),
      description:
        'Role assumed by the WhatsApp Chat Runtime to pull its image, log, call Nova Pro via Converse, reach the AgentCore Gateway, and read/write the shared AgentCore Memory.',
    });
    ...
    const chatRuntime = new bedrockagentcore.CfnRuntime(this, 'ChatRuntime', {
      agentRuntimeName: 'whatsapp_chat_runtime',
      description:
        'WhatsApp Chat Runtime - Strands + Bedrock Converse + Nova Pro, multimodal text+image+file, ARM64, managed (PUBLIC) network mode',
      ...
      // Default/managed network mode (PUBLIC): the Chat Runtime is a
      // request/response text runtime with no outbound UDP / TURN / WebRTC
      // need, so it does NOT use the VPC-only mode (that is the Call Runtime's
      // requirement). AgentCore manages placement across its supported AZs.
    ...
    cdk.Tags.of(chatRuntime).add('DeploymentPrefix', prefix);
    cdk.Tags.of(chatRuntime).add('Feature', 'whatsapp-restaurant-ai-host');

    // The image must be pushed (build waiter SUCCEEDED) before CFN creates the
    // runtime that references `<repoUri>:latest`.
    chatRuntime.node.addDependency(buildTrigger);

    // ----------------- Outputs (NO exportName per the isolated-app rule) ----
    new cdk.CfnOutput(this, 'ChatRuntimeArn', {
      value: chatRuntime.attrAgentRuntimeArn,
      description:
        'WhatsApp Chat Runtime ARN - consumed by the webhook stack as ChatRuntimeArn (CfnParameter). No exportName per the isolated-app rule.',
    });

    new cdk.CfnOutput(this, 'ChatAgentEcrRepoUri', {
      value: repo.repositoryUri,
      description:
        'ECR repo URI for the Chat Runtime image - observability and deploy-all threading.',
    });

    new cdk.CfnOutput(this, 'ChatAgentCodeBuildProjectName', {
      value: project.projectName,
      description: 'CodeBuild project name that builds the ARM64 Chat Runtime image (observability).',
    });

    new cdk.CfnOutput(this, 'ChatAgentSourceBucketName', {
    ...
    new cdk.CfnOutput(this, 'AgentCoreAzContract', {
      ...
      description:
        'Echo of the shared AZ-resolution contract (Bedrock-supported AZ IDs). The Chat Runtime uses managed (PUBLIC) network mode so it is not bound to a subnet; emitted for cross-runtime uniformity and operator observability.',
    });
    ...
    // Stack-level, mirroring the telephony build/runtime bin suppressions.
