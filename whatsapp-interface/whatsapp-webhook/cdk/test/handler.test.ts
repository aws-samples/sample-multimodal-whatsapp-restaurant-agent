...
// VoiceNotes reply selection moved OUT of the worker (async-reply-delivery
// Move B): the VoiceNotes Runtime now owns delivery and chooses audio vs. text
// fallback itself (handler._deliver_voice_result), delivering via the Sender
// Lambda. The worker's audioHandler is dispatch-only, so the former
// chooseVoiceReply routing test lives in the runtime's test suite
// (backend/agentcore-runtime-voicenotes/agent/tests/test_async_delivery.py).
